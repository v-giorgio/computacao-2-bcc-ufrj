...
public class JogoOnlineTest {
    ...
    @Test
    public void testarObterRankingJogadores() throws SenhaInvalidaException, UsuarioInexistenteException {
        JogoOnline jogo = new JogoOnline();
        Jogador jogadoraMaria = jogo.cadastrarJogador("maria", "3456");
        Jogador jogadorJoao = jogo.cadastrarJogador("joao", "1234");
        jogo.fazerLogin("maria", "3456");
        ...
        Partida partida2 = jogo.iniciarPartida(jogadorJoao, jogadoraMaria);
        jogo.encerrarPartida(partida2, 1);
        System.out.println(jogo.obterRanking());

    }

    @Test
    public void testarObterJogadoresOrdemAlfabetica() throws SenhaInvalidaException, UsuarioInexistenteException {
        JogoOnline jogo = new JogoOnline();
        Jogador jogadoraMaria = jogo.cadastrarJogador("maria", "3456");
        Jogador jogadorJoao = jogo.cadastrarJogador("joao", "1234");
        Jogador jogadorZe = jogo.cadastrarJogador("ze", "7890");
        Jogador jogadoraAna = jogo.cadastrarJogador("ana", "0001");

        System.out.println(jogo.obterJogadoresEmOrdemAlfabetica());
        assertEquals("ana", jogo.obterJogadoresEmOrdemAlfabetica().get(0));
        assertEquals("joao", jogo.obterJogadoresEmOrdemAlfabetica().get(1));
        assertEquals("maria", jogo.obterJogadoresEmOrdemAlfabetica().get(2));
        assertEquals("ze", jogo.obterJogadoresEmOrdemAlfabetica().get(3));
    }
}
